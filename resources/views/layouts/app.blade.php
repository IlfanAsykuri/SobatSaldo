        ...
        if ('serviceWorker' in navigator) {
            ...
            window.addEventListener('online', () => {
                navigator.serviceWorker.ready.then(reg => {
                    if(reg.sync) reg.sync.register('sync-transactions');
                });
            });
        }
    </script></head>
<body class="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-300 overflow-x-hidden">
            ...
            <div class="px-4 py-4 border-t border-slate-100 dark:border-slate-700/50">
                <div class="flex items-center gap-3 px-2">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">Ilfan Asykuri</p>
                        <p class="text-xs text-slate-400 truncate">ilfan@sobatsaldo.id</p>
                    </div>
                </div>
            </div>
        </aside>
    ...
    <div class="lg:hidden flex flex-col min-h-screen">
        ...
        <nav class="fixed bottom-0 left-0 right-0 z-30 bg-white/95 dark:bg-slate-800/95 backdrop-blur-md shadow-[var(--shadow-bottom-nav)] safe-area-bottom">
            <div class="flex items-center justify-around px-2 py-2 max-w-lg mx-auto">
                ...
                <a href="{{ route('app.workspace') }}"
                   ...
                   class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-2xl transition-all duration-200 min-w-0
                          {{ request()->routeIs('app.workspace') ? 'text-emerald-600' : 'text-slate-400 hover:text-slate-600' }}">
                    <svg class="w-6 h-6" fill="{{ request()->routeIs('app.workspace') ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="{{ request()->routeIs('app.workspace') ? '0' : '2' }}">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                    </svg>
                    <span class="text-[10px] font-semibold">Catat</span>
                </a>
